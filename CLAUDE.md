...
- **Scraping sources**: `SourceExternal` (in `entity_manga_external`) defines a plugin contract for non-MangaDex sources (AsuraScan, MangaClash). Mangadex is `builtIn` and goes through `manga_dex_api`; its source use-case getters `throw UnimplementedError()`.
