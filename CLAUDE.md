...
- **Web Filesystem Adapters Are Persistent via `persistent_file_system` (fs_shim/IndexedDB)**
  - **Location**: `module/library/persistent_file_system/` + the two web adapters (`module/library/manga_service_drift/lib/src/database/adapter/filesystem/filesystem_web.dart`, `module/core/core_storage/lib/src/manager/path_manager/adapter/filesystem_web.dart`)
  - **Context**: issue #104 — both web adapters used `MemoryFileSystem` (drift's built a fresh one per call), so `FileDao` rows were dead on arrival and backups vanished on reload. They now return fixed roots (`/database`, `/local`) of the shared `persistentFileSystem` singleton (`FsShimFileSystem` over fs_shim's `fileSystemWeb`, i.e. IndexedDB; pure-Dart `newFileSystemMemory()` backs the tests and exercises the same `FileSystemIdb` code path). The wrapper implements the full `package:file` surface; **sync members, links, watch, `File.open()` (RandomAccessFile), and `createTemp` throw `UnsupportedError`** (fs_shim is async-only); fs_shim errors are rethrown as dart:io `FileSystemException` so consumers keep catching one type.
  - **Troubleshooting**: Web file bugs → check both adapters still share the `persistentFileSystem` singleton (two instances = two IndexedDB connections). Browser-quota exhaustion surfaces as `FileSystemException`. Multi-tab writes are last-write-wins (no coordination). Never use `*Sync` members or `File.open()` on these roots. fs_shim is pinned `^2.4.0` because ≥2.4.0+1 requires Dart ≥3.9 while the project SDK is 3.8.1 — on an SDK bump it floats to 2.5.x (interface verified identical). Image-cache persistence on web is still unsupported (flutter_cache_manager's web `Config` defaults to in-memory `MemoryCacheSystem` + `NonStoringObjectProvider`) — `Config(fileSystem:)` is the future injection point for that.
- **CustomCacheStore Is a Fork of Upstream CacheStore — Eviction Is Rescue-then-Delete**
  - **Location**: `module/core/core_storage/lib/src/manager/custom_cache_manager/custom_cache_store.dart`
  - **Context**: `CustomCacheStore` forks flutter_cache_manager's `CacheStore`. Upstream deletes evicted files inline in `_removeCachedFile`; this fork runs an awaited `rescueEvictedFile` hook **before** deleting (both failures logged and swallowed) — every evicted file ends up deleted, no branch on a flag (issue #103: rows were deleted but files leaked forever for every cache). `ImagesCacheManager` is the sole hook user: the rescue copies to `FileDao.addFromFile` so evicted images are persisted into the app DB; the old `deleteFileEvent` → `JobManager` `persistentImage` job pipeline was removed (the handler remains only to drain legacy queued rows). `_cleanupCache`/`emptyCache` await file deletions **before** `provider.deleteAll` (deliberate divergence from upstream) so a crash cannot orphan files with no index rows.
  - **Troubleshooting**: If evicted images stop appearing in the DB, check the `rescueEvictedFile` closure passed by `ImagesCacheManager`. If a cache leaks files on eviction, ensure the eviction paths go through `_removeCachedFile` (all three do: `removeCachedFile`, `emptyCache`, scheduled cleanup). If eviction tests hang, remember `_scheduleCleanup` arms a real timer — drive it with `fakeAsync` + `flushTimers` and `cleanupRunMinInterval = Duration.zero`. Also, mocktail needs `registerFallbackValue` for `Duration` and `CacheObject` when stubbing with `any()`.

- **Schema v3 Enforces Foreign Keys with ON DELETE CASCADE (#137)**
  - **Location**: `module/library/manga_service_drift/lib/src/tables/*` + `database.dart` (`beforeOpen`, `from2To3`)
  - **Context**: since 2026-09-23 the child tables carry real FKs (`chapter_tables.manga_id → manga_tables.id`, `image_tables.chapter_id → chapter_tables.id`, `library_tables.manga_id`, `relationship_tables.(tag_id, manga_id)`) with `ON DELETE CASCADE`, and `beforeOpen` runs `PRAGMA foreign_keys = ON` (per-connection, covers IO and the wasm/IndexedDB web executor). `manga_id` on chapters stays nullable — chapters without a manga are legal (legacy syncs). The v2→v3 migration first deletes legacy orphans (dirty pre-#136 installs) then rebuilds the four tables via `m.alterTable(TableMigration(...))` because SQLite cannot ALTER-add constraints. DAO cascade simplification: `MangaDao.remove`/`ChapterDao.remove` no longer hand-delete children — the DB cascades them — but they must **collect child rows (for file_tables cleanup) before the parent delete**, since cascades make them unreadable afterwards. `file_tables` is webUrl-keyed with no FK and stays app-managed.
  - **Troubleshooting**: `SqliteException(787) FOREIGN KEY constraint failed` on insert ⇒ the fixture/flow wrote a child before its parent — audit ordering (all current sync flows are parent-first). Tests that intentionally seed orphans (diagnostics, which watch dirty *restored* files) or drop tables must toggle `PRAGMA foreign_keys = OFF` first and re-enable after. The drift_dev-generated `migration_test.dart` must start from `verifier.schemaAt(fromVersion).newConnection()` — a fresh `AppDatabase` runs `onCreate` at the *latest* schema and fails validation against older references.

- **`melos run <script>` Prompts (and Crashes) in Non-TTY Shells When the Script Has `packageFilters`**
  - **Location**: `melos.yaml` (`test:`, `generate:`, `clean:coverage:` … any script with `packageFilters`) — melos 6.3.3
  - **Context**: melos 6.3.3's `run` command interactively asks "Select a package to run the X script" for every script that defines `packageFilters` (see `_RunMixin._runScript` → `promptChoice`), even though the exec form fans out to all filtered packages. In a non-TTY context (CI step, background agent shell) the prompt crashes with `StdinException: Error getting terminal echo mode` and the run ends with the misleading message `Failed to update packages.` — **while still exiting 0**. Observed 2026-09 verifying the #105–#107 stack: `melos run test` in a background shell ran nothing. `--no-select` is not a safe workaround (it picks the first choice, which for multi-package scripts is the green `*` "all" entry but is easy to get wrong).
  - **Troubleshooting**: In scripts/CI/agents, invoke the exec equivalent instead of `melos run`: e.g. the `test` script equals `melos exec --concurrency 5 --fail-fast --order-dependents --dir-exists=test -- "flutter test"` (copy the `exec:` + `packageFilters` block of the script in `melos.yaml`). `melos run analyze` is unaffected (no `packageFilters`). Never trust exit code alone for `melos run` — read the output for the `SUCCESS`/`FAILED` banner and per-package lines.

- **Manual-Web Verification: Serve `build/web` + Hash Routes + IndexedDB Store Counts**
  - **Location**: PR test plans' "manual Chrome smoke" items (macOS desktop build is blocked, see the Pod entry above)
  - **Context**: used to verify #104's backup → hard-reload → restore round-trip (2026-09-22) without a desktop build. `fvm flutter build web`, then serve the bundle statically (`python3 -m http.server <port> --directory build/web`) and open it in a browser. Navigation does not need the UI: go_router uses the default hash strategy, so deep links like `/#/setting/data_storage` (paths in each feature module's `route_path.dart`, e.g. `MoreRoutePath.dataStorage`) land directly on the screen. Flutter web renders to canvas (no DOM/ARIA tree for scrapers), so assert state from the page context instead: count `indexedDB` object-store rows via injected JS — `lfs.db` (fs_shim's web filesystem: `file`/`part`/`tree` stores) for the `persistent_file_system` singleton, plus the drift database's own IndexedDB name for DB content. Those counts corroborate every UI step (file written, survived reload, restored) without OCR/screenshots.
  - **Troubleshooting**: If an embedded/headless browser screenshot never changes (byte-identical captures), the pane isn't repainting — toggling the viewport size by 1px before each capture forces a fresh frame. Clicks synthesized at the DOM level still reach Flutter. Restore finishes with `WrapperScreen.restart`, which may come back at a different device-pixel ratio — re-measure coordinates after any restart.
