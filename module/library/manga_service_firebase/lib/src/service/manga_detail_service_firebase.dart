...
class MangaDetailServiceFirebase {
  ...
  Future<MangaFirebase> update({
    required String? key,
    required AsyncValueUpdater<MangaFirebase> update,
    required AsyncValueGetter<MangaFirebase> ifAbsent,
  }) async {
    if (key == null) {
  ...
  Future<List<MangaFirebase>> search({
    ...
    do {
      ...
      data.addAll(
        result.docs
            .map((e) => MangaFirebase.fromJson(e.data()).copyWith(id: e.id)),
      );
