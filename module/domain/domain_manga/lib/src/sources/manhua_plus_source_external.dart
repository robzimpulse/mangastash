...
/// Manhua Plus (https://manhuaplus.com) external manga source.
///
...
/// Extracts the series synopsis from the Madara `div.description-summary`
/// block, falling back to `div.summary__content p`.
String? _summary(Document root) {
  return root.querySelector('div.description-summary')?.text.trim() ??
      root.querySelector('div.summary__content p')?.text.trim();
}
...
class _SearchMangaSourceExternalUseCase
  ...
  Future<bool?> haveNextPage({required Document root}) async {
    ...
    // `a.next.page-numbers` link, once more results exist.
...
class _ListTagSourceExternalUseCase implements ListTagSourceExternalUseCase {
  ...
  Future<List<TagScrapped>> parse({required Document root}) async {
    // Genres appear as <a href="/manga-genre/{slug}"> links on the genre index
